...
# Create log file 
log_file <- xFile
logger <- logger("INFO", appenders = file_appender( xFile) )
...
xstr_date1 = paste0("'",as.character(today() - lubridate::days(30) ),"'")
...
# New lines based on prfiles vs list------- 
df_profile <- get_swt_profiles( conid = conid )

# Manuel screen to keep active techno 
vec_active = c("AH018" , "C11N",  "XH018", "XR013",   "XT011" , "XT018")

df_profile <- df_profile %>% 
  dplyr::filter(techno %in% vec_active)


# Print number to csv to generate 
print(paste( "Nb of pnp-version in profiles :", nrow(df_profile)))
...
if( nrow(df_profile) ==0 ){ 
  log4r::error("No profiles --> check dmp.t_profile_maps_master ") 
} else { # !(nrow(dt_profile) ==0 ) 
  # Loop on dt_profiles rows 
  
  for(i in 1:nrow(df_profile)){
    str_param <- ""
    str_techno <- ""
    str_npnpid <- ""
    str_version <- ""
  ...
  log4r::info(logger, "END step i fin " ) 
  }
} # end else 

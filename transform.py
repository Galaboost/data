...
def build_symaro_reference(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
...
def normalize_symaro_reference(df_route_from_symaro: pd.DataFrame) -> pd.DataFrame:
...
def format_delta_frame(df: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame(
        {
...
def build_reference_delta(
    df_from_datamart: pd.DataFrame, df_route_from_symaro: pd.DataFrame

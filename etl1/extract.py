...
def read_sql_df(engine, query):
    with engine.connect() as connection:
        return pd.read_sql(text(query), connection)


def latest_rows(df, key_columns, time_column):
...
def extract_symaro_data(symaro_engine):
...
def extract_datamart_reference(dmp_engine):
    df = read_sql_df(dmp_engine, "SELECT * FROM t_mes_ref_oper")
